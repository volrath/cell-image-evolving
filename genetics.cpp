...
//     cout << dna[i] << endl;
  ...
  for (int i = 0; i < IMAGE_WIDTH; i++)
    for (int j = 0; j < IMAGE_HEIGHT; j++) {
      ...
      diff += abs((unsigned short int) pr.redQuantum()   - (unsigned short int) px_original[i][j].r);
      diff += abs((unsigned short int) pr.greenQuantum() - (unsigned short int) px_original[i][j].b);
      diff += abs((unsigned short int) pr.blueQuantum()  - (unsigned short int) px_original[i][j].g);
    }
